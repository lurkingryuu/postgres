/* contrib/pg_cedar/pg_cedar--1.0--1.1.sql */

-- complain if script is sourced in psql, rather than via ALTER EXTENSION
\echo Use "ALTER EXTENSION pg_cedar UPDATE TO '1.1'" to load this file. \quit

/* ========================================================================
 * 1. Extend cedar_policies with lifecycle / versioning columns
 * ======================================================================== */
...
ALTER TABLE cedar_policies
    ...
    ADD COLUMN prev_active_id integer     REFERENCES cedar_policies(id),
    ADD COLUMN activated_by   text,
    ADD COLUMN activated_at   timestamptz;

-- Backfill hash for rows that were loaded under v1.0
UPDATE cedar_policies SET policy_hash = md5(policy_text) WHERE policy_hash IS NULL;
...
'Registry of ABAC attribute definitions (name, type, and target entity kind).';

/* ========================================================================
 * 3. Principal attribute store
 * ======================================================================== */
...
'Per-principal ABAC attribute assignments used to enrich Cedar entities.';

/* ========================================================================
 * 4. cedar_validate_text — offline validation without touching the live engine
 * ======================================================================== */
...
/* ========================================================================
 * 5. ABAC governance SQL functions
 * ======================================================================== */

-- ---------------------------------------------------------------------------
-- abac_stage_policyset(name, text) → text
-- ---------------------------------------------------------------------------
    ...
    ELSE
        UPDATE cedar_policies
           SET policy_text  = p_policy_text,
               ...
               is_active    = false,
               created_at   = now()
         WHERE name = p_name AND id = v_existing.id;
...
-- ---------------------------------------------------------------------------
-- abac_validate_policyset(name) → text
-- ---------------------------------------------------------------------------
...
BEGIN
    ...
    -- Use the most-recently activated schema, if any
...
'Validate a named policy (any status) against the active schema using a throw-away engine.';

-- ---------------------------------------------------------------------------
-- abac_activate_policyset(name) → text
-- ---------------------------------------------------------------------------
...
DECLARE
    ...
    v_result    text;
BEGIN
    -- Locate the row to activate (prefer staged, fall back to any non-archived)
    ...
    IF NOT FOUND THEN
        -- Already active or nothing to activate
    ...
    -- Remember current active row for rollback support
    ...
    -- Archive all currently active/shadow rows for this name
    UPDATE cedar_policies
       SET status    = 'archived',
           is_active = false
     WHERE name = p_name AND status IN ('active', 'shadow');

    -- Promote the target row to active
    UPDATE cedar_policies
       SET status         = 'active',
           is_active      = true,
    ...
    -- Reload the live engine and flush the decision cache
...
-- ---------------------------------------------------------------------------
-- abac_rollback_policyset(name) → text
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- abac_shadow_policyset(name) → text
-- Shadow mode: policy is evaluated and logged but decisions are not enforced.
-- The pg_cedar hook checks status='shadow' and treats the result as advisory.
-- ---------------------------------------------------------------------------
...
'Put a policy into shadow mode — it is evaluated and audited but not enforced.';

-- ---------------------------------------------------------------------------
-- abac_effective_policies() → SETOF record
-- ---------------------------------------------------------------------------
...
'List all currently active policysets with version and activation metadata.';

-- ---------------------------------------------------------------------------
-- abac_explain_decision — convenience wrapper
-- ---------------------------------------------------------------------------
...
'Explain a Cedar authorization decision (wrapper around cedar_explain_decision).';
...
BEGIN
    ...
    RETURN 'attribute defined: ' || p_name || ' (' || p_type || ', applies_to=' || p_applies_to || ')';
END;
$$;
...
CREATE FUNCTION abac_get_principal_attrs(p_principal text,
    OUT attr_name  text,
    OUT attr_value text,
...
'Full lifecycle history for all policysets (all statuses, all versions).';
